	...
	case MENU_KEYBOARD_CONFIG:
		...
		else /* change key */
		{
			...
			else if (keyboardGetInput(&keyboardInput))
			{
				...
				{
					if (keySettings[i] == keyboardInput.scancode)
					{
				...
				if (keyboardInput.scancode != SDLK_ESCAPE && // reserved for menu
				    keyboardInput.scancode != SDLK_F11 &&    // reserved for gamma
				    keyboardInput.scancode != SDLK_p)        // reserved for pause
				{
					JE_playSampleNum(S_CLICK);

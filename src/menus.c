...
bool difficultySelect( void )
{
	...
	for (; ; )
	{
		...
		{
			const SDLKey lordKeys[] = { SDLK_l, SDLK_o, SDLK_r, SDLK_d };
			...
			if (newkey)
			{
				...
				if (lordProgress < COUNTOF(lordKeys) &&
					lastkey_sym == lordKeys[lordProgress])
				{
					lordProgress += 1;
		...
		{
			if (menuItemsVisibleCount == 4 && keysactive[SDLK_RIGHTBRACKET])
				menuItemsVisibleCount = 5;
			if (menuItemsVisibleCount == 3 && keysactive[SDLK_g])
